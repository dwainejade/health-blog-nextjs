...
/**
 * UPDATED: Handles image upload with Cloudinary integration
...
export const handleImageUpload = async (file, onProgress, abortSignal) => {
  try {
    ...
    if (cloudName && uploadPreset) {
      console.log("📤 Uploading to Cloudinary via tiptap-utils...");

      const formData = new FormData();
      formData.append("file", file);
      formData.append("upload_preset", uploadPreset);
      // Remove transformation for unsigned uploads - these need to be configured in the upload preset instead
    ...
    // if (imgbbApiKey) {
    //   console.log("📤 Uploading to ImgBB via tiptap-utils...");
    ...
    //   console.log(
    //     "✅ ImgBB upload successful via tiptap-utils:",
    //     data.data.url
    //   );
    ...
    console.warn("⚠️ No cloud storage configured, using blob URL fallback");
    ...
    console.warn(
      "💡 To use cloud storage, add VITE_CLOUDINARY_CLOUD_NAME and VITE_CLOUDINARY_UPLOAD_PRESET to your .env file"
    );

    return blobUrl;
  } catch (error) {
    console.error("Image upload failed in tiptap-utils:", error);
